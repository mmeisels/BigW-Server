...
function findAll(limit) {
    return db.query('SELECT name, description, image__c AS image, productPage__c AS productPage, publishDate__c AS publishDate FROM salesforce.product2 WHERE family=$1 ORDER BY publishDate DESC LIMIT $2', [config.productFamily, limit]);
};
